...
_Magic variables_ have values when provided by the system during certain code events. **It** is the most commonly used mvar, populated by most iterators and comprehensions, as well as the **exists** conditional and **with** contextualizer.
...
Optional. When using the compiler, native Javascript files of objects created with the **main** adjective will automatically instantiate a single copy of the object when the file is required. In other words, instead of simply defining a prototype when the `.js` file is required, an object is also instantiated (which causes any **Instantiate** task to run, thus conceivably launching a program). 
