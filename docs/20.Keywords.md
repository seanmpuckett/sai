...
**Every** is a general-purpose iterator that can handle any collection -- array, object or iterable -- or a single value (which case it executes the block once).  
...
_Note:_ there is a slight performance penalty associated with using __every__ instead of a more specific iterator, as SAI must wrap the collection in order to generically handle all cases. This should not be of a concern for most programs, and it is recommended, for clarity, to use __every__ when possible.
...
The `iterate ... using [function]` calls the given function with two parameters, the iteration value and the current row number:
