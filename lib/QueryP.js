...
var __context={"loader":"SAI.GetSourceFromFilename","path":"src/Construct.sai","workdir":"/Users/seanmpuckett/Documents/ProjSync/Node/sai/src","mtime":"2018-04-04T23:09:42.435Z","fetched":"2018-04-07T15:20:51.987Z"};
...
var __context={"loader":"SAI.GetSourceFromFilename","path":"src/QueryP.sai","workdir":"/Users/seanmpuckett/Documents/ProjSync/Node/sai/src","mtime":"2018-04-04T23:08:40.180Z","fetched":"2018-04-07T15:20:51.784Z"};
var __path=require('path');
var __require=function(fn) {
  try {
    return require(fn);
...
prototype['FilterPiper'] = function(p) {
  var $1_this, $2_trial, $3, $ = this['FilterPiper'] ? this : $bindfail('FilterPiper'),
    $_c, $_d, _fn, _outs, _pattern, _tools, _vars, _work;
  $AI.expectsThrow(p, {
    "clauses": true,
    "tools": true
  }, '\'FilterPiper\''); {
...
prototype['LowestPiper'] = function(p) {
  var $4_this, $5_trial, $6_trial, $7, $ = this['LowestPiper'] ? this : $bindfail('LowestPiper'),
  ...
  $AI.expectsThrow(p, {
    ...
    _ascending = 1;
    _work = p.clauses[0];
    if ($5_trial = ((p.clauses.length > 1))) {
      return ({
        error: 'SAI Compiler: ' + _work.clause + ' should not have multiple clauses.'
      });
    }
    if ($6_trial = ((_work.clause === 'highest'))) {
      _ascending = (0 - (1));
    }
    switch ($7 = (_work.type)) {
      case 'lowest.expr':
...
prototype['SortPiper'] = function(p) {
  var $10_key, $11_sum, $12, $13_intothis, $14_intokey, $15_intosum, $16_this, $17_key, $18_this, $19_key, $20_trial, $21_trial, $22_trial, $23_trial, $24, $8_this, $9_this, $ = this['SortPiper'] ? this : $bindfail('SortPiper'),
  ...
  $AI.expectsThrow(p, {
    ...
    _tools = [];
    _Repl = p.tools.replacer;
    _grammar = $AI.sum_op(p.clauses, function($11_sum, $9_this, $10_key) {
      var $t = $9_this.clause;
      return (undefined === $11_sum) ? $t : ($11_sum + $t);
    }, undefined);
    _work = p.clauses[0];
    switch ($24 = (_work.type)) {
      case 'by.expr':
        try {
          _keyed = false;
          _pattern = $AI.alter_op($AI.sum_op($AI.map_op($AI.sum_op(p.clauses, function($15_intosum, $13_intothis, $14_intokey) {
            switch ($12 = ($13_intothis.clause)) {
              case 'by':
                if (!(_keyed)) {
                  _keyed = ($13_intothis.args.expr.indexOf('\x5e{key}') >= 0);
                }
                $15_intosum.push({
                  expr: ((undefined !== ($_b = $13_intothis.args.expr)) ? $_b : '\x5e{this}'),
                  order: 1
                });
                break;
              case 'asc':
                (Array.isArray($_b = $15_intosum) ? $_b[$_b.length - 1] : $AI.element_op($AI.slice_op($_b, undefined, -1), 0)).order = 1;
                break;
              case 'desc':
                (Array.isArray($_b = $15_intosum) ? $_b[$_b.length - 1] : $AI.element_op($AI.slice_op($_b, undefined, -1), 0)).order = (0 - (1));
                break;
              default:
            ...
            return $15_intosum;
          }, []), function($16_this, $17_key) {
            return 'aa=' + _Repl($16_this.expr, {
              'this': ((_keyed) ? ('a[1]') : ('a')),
              'key': 'a[0]'
            }) + ';' + '\n' + 'bb=' + _Repl($16_this.expr, {
              'this': ((_keyed) ? ('b[1]') : ('b')),
              'key': 'b[0]'
            }) + ';' + '\n' + 'if (aa<bb) return ' + (0 - ($16_this.order)) + '; if (aa>bb) return ' + $16_this.order + ';';
          }), function(s, i) {
            return (undefined === s) ? i : s + i;
          }, undefined), function($18_this, $19_key) {
            if (!(_keyed)) {
        ...
        break;
      case 'by.outblock':
        if ($21_trial = ((p.clauses.length > 1))) {
          return ({
        ...
        break;
      case 'by.use':
        if ($22_trial = ((p.clauses.length > 1))) {
          return ({
        ...
        break;
      case 'by.none':
        if ($23_trial = ((p.clauses.length > 1))) {
          return ({
...
prototype['LimitPiper'] = function(p) {
  var $25_this, $26_trial, $27_trial, $28, $ = this['LimitPiper'] ? this : $bindfail('LimitPiper'),
    $_c, $_d, _ct, _pattern, _st, _tools, _work;
  $AI.expectsThrow(p, {
    "clauses": true,
    "tools": true
  }, '\'LimitPiper\''); {
    ...
    _tools = [];
    _work = p.clauses[0];
    if ($26_trial = ((p.clauses.length > 1))) {
      return ({
        error: 'SAI Compiler: ' + _work.clause + ' should not have multiple clauses.'
      });
    }
    switch ($28 = (_work.type)) {
      case 'limit.exprs':
        if ($27_trial = ((_work.args.exprs.length === 1))) {
          _st = undefined;
...
prototype['Documents'] = function(p) {
  var $29_this, $ = this['Documents'] ? this : $bindfail('Documents'); {
    return ({
      'pipers': {
        'limit': {
