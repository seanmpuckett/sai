...
// Javascript source for PipersC.sai transpiled by SAI
//
...
var __context={"name":"Construct","loader":"SAI.GetSourceFromPaths","path":"src/Construct.sai","mtime":"2018-03-07T14:21:44.851Z","fetched":"2018-03-09T18:27:58.385Z"};
...
var __context={"name":"PipersC.sai","loader":"SAI.GetSourceFromPaths","path":"src/PipersC.sai","mtime":"2018-03-09T18:25:10.112Z","fetched":"2018-03-09T18:27:58.243Z"};
var isa = prototype.isa = 'PipersC';
var $bindfail = function(m) {
  throw new Error("SAI: A call to " + m + " on object " + isa + " has come unbound from any instance of that object. (If this is not in error, mark the declaration of " + m + " as unbound.)");
}
prototype.isof['PipersC'] = {
  version: '0.0.0-unspecified',
  isa: isa,
  context: __context
};
  ...
  $AI.expectsThrow(p, {
    ...
    switch ($1 = (p.work.type)) {
      ...
      case 'total.expr':
        ...
        _pattern = '$AI.reduce_op_fast(%VALUE%,' + _fn + ',' + _startvalue + ')';
        break;
      ...
      case 'into.valuse':
      case 'total.use':
        _pattern = '$AI.reduce_op_fast(%VALUE%,' + p.work.args.use + ',' + _startvalue + ')';
        break;
      default:
        return ({
    ...
    return ({
      leftright: _pattern.split('%VALUE%'),
      tools: _tools
    });
  }
};
  ...
  $AI.expectsThrow(p, {
    ...
    switch ($3 = (p.work.type)) {
      case 'total.expr':
        ...
        _fn = p.tools.replacer(_fn, _vars);
        _pattern = '$AI.reduce_op_fast(%VALUE%,' + _fn + ',undefined)';
        break;
      case 'total.use':
        _pattern = '$AI.reduce_op_fast(%VALUE%,' + p.work.args.use + ',undefined)';
        break;
      case 'total.none':
        _pattern = '$AI.reduce_op_fast(%VALUE%,function(s,i){' + '\n' + '  return (undefined===s) ? i : s+i;' + '\n' + '},undefined)' + '\n' + '';
        break;
      default:
        return ({
...
prototype['AuditPiper'] = function(p) {
  ...
  $AI.expectsThrow(p, {
    ...
    switch ($5 = (p.work.type)) {
      ...
      case 'audit.use':
      case 'observe.use':
        _pattern = '$AI.' + _op + '(%VALUE%,' + p.work.args.use + ')';
        break;
      default:
        return ({
...
prototype['ArraysPiper'] = function(p) {
  ...
  $AI.expectsThrow(p, {
    ...
    switch ($9 = (p.work.type)) {
      case 'iterate.none':
        _pattern = '$AI.iterate_op(%VALUE%)';
        break;
      case 'concat.expr':
        _pattern = '$AI.concat_op(%VALUE%,' + p.work.args.expr + ')';
        break;
      case 'collect.none':
        _pattern = '$AI.collect_op(%VALUE%)';
        break;
      case 'enlist.none':
        _pattern = '$AI.enlist_op(%VALUE%)';
        break;
      case 'entrait.none':
        _pattern = '$AI.entrait_op(%VALUE%)';
        break;
      default:
        return ({
...
prototype['KVPiper'] = function(p) {
  ...
  $AI.expectsThrow(p, {
    ...
    switch ($11 = (p.work.type)) {
      case 'keys.none':
        _pattern = '$AI.keys_op(%VALUE%)';
        break;
      case 'values.none':
        _pattern = '$AI.values_op(%VALUE%)';
        break;
      default:
        return ({
...
prototype['ReplicatePiper'] = function(p) {
  ...
  $AI.expectsThrow(p, {
    ...
    switch ($13 = (p.work.type)) {
      case 'replicate.expr':
        _pattern = '$piper$replicate((%VALUE%),(' + p.work.args.expr + '))';
        break;
      default:
        return ({
